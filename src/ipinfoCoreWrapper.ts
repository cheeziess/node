...
export default class IPinfoCoreWrapper {
    ...
    /**
     * Creates IPinfoCoreWrapper object to communicate with the IPinfo Core API.
     *
    ...
    constructor(
        ...
        this.timeout =
            timeout === null || timeout === undefined
                ? REQUEST_TIMEOUT_DEFAULT
                : timeout;
        this.baseUrl = baseUrl || `https://${HOST_CORE}`;
    }
    ...
    public async lookupIp(
        ...
        const data = await this.cache.get(IPinfoCoreWrapper.cacheKey(ip));
        ...
        return this.fetchApi(ip).then(async (response) => {
            const ipinfo = (await response.json()) as IPinfoCore;
